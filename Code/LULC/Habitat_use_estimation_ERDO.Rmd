...
```{r}
DF<-read.csv(here("Data", "FinalHR", "ERDO_LULC_HR_subset.csv"))#actual habitat use
```
...
A_Avg_percent_area<-All %>% group_by(LandcoverNA, Treatment) %>% summarise(
     Mean_percent = round(mean(PercentArea, na.rm = T),2),
     sd_percent = round(sd(PercentArea, na.rm = T), 2)
)
```
And plot:
